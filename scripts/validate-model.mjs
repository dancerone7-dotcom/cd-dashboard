...
for(const key of Object.keys(M.DEMO_ARCHETYPES)){
  ...
  const demoWheelAxes=M.capacityWheelDimensions(demoStory);check((demoWheel.match(/data-goal-outlook=/g)||[]).length===10&&(demoWheel.match(/data-wheel-axis=/g)||[]).length===(demoWheelAxes.length>=3?demoWheelAxes.length:0),`${key}: Capacity Wheel is not using eligible axes from the same 10 selected goals.`);
...
check(wheelDimensions.every(item=>finite(item.wheelCurrentPct)&&finite(item.wheelProjectedPct)&&finite(item.currentPct)&&finite(item.projectedPct)),'Capacity Wheel included a support-only or non-confidence-qualified fixed-demand axis.');
...
check(!/100% is today|relative to today|supporting capacities are relative/i.test(wheelHTML),'Capacity Wheel still mixes today-normalized support axes with fixed-demand axes.');
const insufficientWheelStory={topTrainingPriorities:[{metric:'direct-a',chartBasis:'goal-demand',wheelCurrentPct:100,wheelProjectedPct:80},{metric:'support-a',chartBasis:'today',currentPct:100,projectedPct:70}],topStrengths:[{metric:'direct-b',chartBasis:'goal-demand',wheelCurrentPct:120,wheelProjectedPct:95}]};
