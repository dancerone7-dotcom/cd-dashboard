...
globalThis.__MODEL__={
  ...
  capacityTrajectories,capacitySummaryHTML,limiterSynthesis,assessmentContext,supportPrioritySynthesis,foundationalPrioritySynthesis,trainingPrioritySynthesis,goalTrainingPrioritySynthesis,patientGoalOutlook,patientStory,foundationalSummaryHTML,
...
check((demoSummary.match(/class="caprankitem"/g)||[]).length<=8,'Patient headline must remain concise.');
...
for(const key of Object.keys(M.DEMO_ARCHETYPES)){M.setPatient(M.createArchetypePatient(key));const story=M.patientStory(allGoals),expected={strength:story.topStrengths.map(x=>x.metric),priority:story.topTrainingPriorities.map(x=>x.metric),foundation:story.wholeBodyOpportunities.map(x=>x.metric)};for(const mode of ['C','A','B','R']){const out=M.reportModeHTML(mode,allGoals),read=kind=>[...out.matchAll(new RegExp(`data-story-${kind}="([^"]+)"`,'g'))].map(match=>match[1]);for(const [kind,metrics] of Object.entries(expected))for(const metric of metrics)check(read(kind).includes(metric),`${key}/${mode}: canonical ${kind} ${metric} is missing.`);check(!patientForbidden.test(out),`${key}/${mode}: model-development language leaked into the patient view.`);}}
