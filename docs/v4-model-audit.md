...
- Expanded the shared goal-priority presentation to the first 12 items from the existing synthesis order: Action groups 1–4 as Highest priority, 5–8 as Secondary priorities, and 9–12 as Additional focus areas; Simple remains at three; Trajectory defaults to eight with an all-priorities reveal; Capacity wheel remains capped at eight axes; and PDF export adds a dedicated full-priority page. These are presentation limits only and do not alter scores or rank.
- Restricted the Capacity wheel to entered capacities normalized against defensible fixed goal demands. Support-only, today-normalized capacities are never mixed into its polygon; fewer than three eligible axes produce an explicit unavailable state. Trajectory preserves the shared priority order while visually distinguishing fixed-goal-need rows from support-only rows.
...
- `node scripts/validate-model.mjs`: PASS - 36 goals, 83 metrics, 7 native-unit archetypes, 5 modes, 12-rank presentation tiers, trajectory expansion, headline diversity, clearance, goal-support and goal-independent foundational sensitivity, calibration breadth, evidence horizons, fixed demand, missing data, import/export, and 39-page print checks.
...
- PDF render QA: PASS - Action-mode 39-page fixture rendered at US Letter landscape; the executive summary and dedicated full-priority page were visually inspected for clipping, overlap, rationale visibility, and legibility.
- Final model-integrity pass: PASS - rechecked all five modes and seven archetypes with no console errors or non-finite output. The current 39-page Letter landscape PDF keeps the Action executive summary concise and adds the full goal-priority list on its own page; both pages were re-rendered without clipping or overlap.
